...
Bank::Bank(int num_accounts) {
  ...
  //Create accounts and initialize all fields of each account
...
int Bank::CheckBalance(int workerID, int accountID){
  ...
  balance = accounts[accountID].balance;//<--CRITICAL SECTION
...
int Bank::deposit(int workerID, int ledgerID, int accountID, int amount) {
   ...
   */
   ...
   accounts[accountID].balance += amount;//<--CRITICAL SECTION
   pthread_rwlock_unlock(&accounts[accountID].lock);
...
int Bank::withdraw(int workerID, int ledgerID, int accountID, int amount) {
  ...
  if(flag){//If ledger was unsuccessful, record failure, return -1
    char buffer[2048];
    pthread_mutex_lock(&bank_lock);
  ...
  else{//If ledger was successful record success, return 0
    char buffer[2048];
    pthread_mutex_lock(&bank_lock);
...
int Bank::transfer(int workerID, int ledgerID, int srcID, int destID,unsigned int amount) {
   ...
   */
   if(srcID == destID){return -1;}//Invalid, return -1
   
   //order locks in terms of account ID's
   if(srcID < destID){
   ...
   //START OF CRITICAL SECTION
   //mutate account fields and set flag accordingly
   ...
   //If unsuccessful ledger, record failure, return -1
   if(flag == 0){
    char buffer[2048];
    pthread_mutex_lock(&bank_lock);
   ...
   else{//If successful, record success, return 0
    char buffer[2048];
    pthread_mutex_lock(&bank_lock);
